...
public class BuscarTiposCertificadosServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			stmt = conn.prepareStatement("select desc_tipo_certificado,nro_tipo_certificado from dbo.tipos_certificados (nolock)where cod_grupo = ?");
            stmt.setString(1, request.getParameter("cod_grupo"));
		    ...
		    while(result.next()) {
				...
				c.setCodGrupo(request.getParameter("cod_grupo"));
				c.setDescTipoCertificado(result.getString("desc_tipo_certificado"));
